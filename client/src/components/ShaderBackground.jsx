/**
 * WebGL mesh-drift shader, from the 21st.dev Shader Builder.
 *
 * Converted from TypeScript to JSX for this project: the type annotations are
 * gone, the logic is verbatim. Zero dependencies, one canvas that fills its
 * parent. Sits behind hero content.
 */
...
// Palette retuned from the stock sky-blue to the console's teal, so the hero
// and the dashboard read as one product.
const UNIFORMS = {
  colors: [
    [0.0392, 0.0470, 0.0549],
    [0.0470, 0.2196, 0.2588],
    [0.3019, 0.7215, 0.7686],
    [0.6549, 0.5411, 0.9803],
    [0.9176, 0.9764, 1],
    [0.9176, 0.9764, 1],
    [0.9176, 0.9764, 1],
    [0.9176, 0.9764, 1],
...
export function ShaderBackground({ className }) {
  ...
  useEffect(() => {
    ...
    // Honour reduced motion: paint one static frame, skip the loop.
    ...
    const cursorOn = UNIFORMS.cursorEnabled && !reduced;
    ...
    function render(now) {
      ...
      gl.uniform4f(uni.cursor, cursorOn ? cursorPresence : 0,
