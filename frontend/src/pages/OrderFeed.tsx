...
import { Rocket, Search, ListFilter, SlidersHorizontal, User, Paperclip, Send } from "lucide-react";
...
function TaskCard({ t, selected, onClick }: { t: any; selected: boolean; onClick: () => void }) {
  return (
    <button
      onClick={onClick}
      className={`w-full text-left card p-4 transition-all ${selected ? "border-white/30 bg-panel2" : "hover:border-white/15"}`}
    >
    ...
    <div className="min-h-screen bg-ink">
      <header className="flex flex-wrap items-center justify-between gap-4 px-8 py-4 border-b divider sticky top-0 bg-ink/90 backdrop-blur z-20">
        <button onClick={() => navigate("/dashboard")} className="flex items-center gap-2.5">
          <span className="h-8 w-8 rounded-full bg-white text-ink grid place-items-center">
            <Rocket size={14} />
          </span>
          <span className="font-display font-semibold">AstraProcure</span>
        </button>
        <div className="flex items-center gap-2 text-xs">
          <span className="text-white/40">Workspace: Operations</span>
          <button className="btn-ghost px-3 py-1.5">Switch</button>
        </div>
        <div className="flex items-center gap-3 text-xs text-white/50">
          <span>Signed in as</span>
          <span className="flex items-center gap-1.5 font-medium text-white">
            <User size={14} /> Elena Marques
          </span>
        </div>
      </header>
            ...
            {FILTERS.map((f) => (
              <button
                ...
                className={`pill transition-all ${
                  activeFilter === f.label ? "bg-white text-ink" : `bg-white/5 text-white/60 hover:bg-white/10`
                }`}
              >
                {f.label}
              </button>
            ))}
            <span className="text-white/30 text-xs ml-2">Date: 2026-08-20 — 2026-08-27</span>
          ...
          <div>
            <Reveal>
              <div className="flex items-center justify-between mb-2">
                ...
                <span className="text-white/30 text-xs">Manual reorder enabled</span>
              </div>
            ...
            <Reveal>
              <div className="flex items-center justify-between mb-2">
                ...
                <span className="text-white/30 text-xs">Auto-prioritized</span>
              </div>
            ...
            <div className="card p-6">
              <div className="flex flex-wrap items-center justify-between gap-3 mb-1">
                ...
                <div className="flex items-center gap-2">
                  ...
                  <span className="text-xs text-white/40">ETA: {taskDetail.eta}</span>
                  <span className="h-8 w-8 rounded-full bg-white/10 grid place-items-center"><User size={14} /></span>
                </div>
              </div>
                  ...
                  <div className="space-y-4">
                    {taskDetail.decisions.map((d) => (
                      <div key={d.title} className="text-xs">
                        ...
                        {d.meta && <div className="text-white/40 mt-0.5">{d.meta}</div>}
                        <div className="text-white/25 mt-1">{d.time}</div>
                      </div>
                    ))}
                  </div>
                </div>
                  ...
                  <div className="space-y-4">
                    {taskDetail.comms.map((c) => (
                      <div key={c.time} className="text-xs">
                        ...
                        <div className="text-white/25 mt-1">{c.time}</div>
                      </div>
                    ))}
                  </div>
                </div>
                  ...
                  <div className="space-y-3">
                    {taskDetail.attachments.map((a) => (
                      <div key={a.name} className="flex items-start gap-2">
                        <Paperclip size={14} className="text-white/40 mt-0.5" />
                        <div className="text-xs">
                          <div className="font-medium">{a.name}</div>
                          <div className="text-white/40">{a.meta} · {a.date}</div>
                        </div>
                      </div>
                    ))}
                  </div>
              ...
              <div className="flex flex-wrap items-center gap-3 mt-8 pt-6 border-t divider">
                ...
                <span className="text-white/30 text-xs">Selected: {selectedId}</span>
                <div className="flex-1 relative min-w-[200px]">
                  <input className="field w-full pl-4 pr-9 py-2.5 text-sm outline-none" placeholder="Write a message or @mention" />
                  <Send size={14} className="absolute right-3 top-1/2 -translate-y-1/2 text-white/40" />
                </div>
                <button className="btn-ghost px-5 py-2.5 text-sm">Cancel</button>
                <button className="px-5 py-2.5 text-sm rounded-full bg-amber text-ink font-semibold hover:opacity-85 transition-opacity">
                  Reroute
                </button>
