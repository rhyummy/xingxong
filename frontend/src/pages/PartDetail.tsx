...
import { ArrowLeft, Copy, Mail, Bookmark, BellPlus, Phone } from "lucide-react";
...
export default function PartDetail() {
  ...
  return (
    <div className="min-h-screen bg-ink">
      <header className="flex items-center justify-between px-8 py-4 border-b divider sticky top-0 bg-ink/90 backdrop-blur z-20">
        <button onClick={() => navigate("/orders")} className="btn-ghost px-4 py-2 text-sm flex items-center gap-2">
          <ArrowLeft size={14} /> Back to Dashboard
        </button>
        <div className="flex items-center gap-3">
          <span className="text-xs text-white/40 btn-ghost px-4 py-2">Workspace: Operations</span>
          <button onClick={() => navigate("/sign-in")} className="btn-ghost px-4 py-2 text-sm">Sign Out</button>
        </div>
      </header>

      <main className="px-8 py-8 max-w-[1600px] mx-auto space-y-6">
        <Reveal>
          <div className="card p-6 flex flex-wrap items-start justify-between gap-6">
            <div className="max-w-2xl">
              ...
              <p className="text-white/45 text-sm leading-relaxed">{part.desc}</p>
            ...
            <div className="text-right">
              ...
              <div className="flex gap-2 mb-4 justify-end">
                ...
                <button className="px-3 py-1.5 text-xs rounded-full bg-white text-ink font-semibold flex items-center gap-1.5"><BellPlus size={12} /> Create Alert</button>
              </div>
              <div className="text-white/40 text-xs">Part Lifecycle</div>
              <div className="text-mint font-semibold text-sm">{part.lifecycle}</div>
              <div className="flex gap-6 mt-4">
                <div>
                  <div className="text-white/40 text-xs">Avg Lead Time</div>
          ...
          <div className="space-y-6">
            <Reveal>
              <div className="card p-5">
                <div className="flex justify-between items-start mb-1">
                  ...
                  <button className="text-xs text-white/40">Manage Suppliers</button>
                </div>
                <p className="text-white/40 text-xs mb-4">Vetted suppliers with historical performance and lead time distributions</p>
                <div className="space-y-4">
                  {part.suppliers.map((s) => (
                    <div key={s.name} className="flex items-center justify-between gap-3">
                      <div>
                        <div className="text-sm font-medium">{s.name}</div>
                        <div className="text-xs text-white/40">Rating: {s.rating} · Region: {s.region}</div>
                        <div className="text-xs text-white/30">Lead time: {s.leadTime}</div>
                      </div>
              ...
              <div className="card p-5">
                <div className="flex justify-between items-center mb-1">
                  ...
                  <div className="flex gap-1 text-xs">
                    {["1Y", "6M", "3M"].map((t) => (
                      <button key={t} className={`px-2 py-1 rounded-full ${t === "3M" ? "bg-white text-ink" : "text-white/40"}`}>{t}</button>
                    ))}
                  </div>
                </div>
                <p className="text-white/40 text-xs mb-4">Last 12 months — high-contrast</p>
                ...
                <div className="flex justify-between text-xs">
                  <span className="text-white/40">Latest price <span className="text-white font-medium">{part.avgPrice}</span></span>
            ...
            <Reveal delay={0.1}>
              <div className="card p-5">
                ...
                <p className="text-white/40 text-xs mb-4">Standard MOQs, active exceptions, and notes</p>
                <div className="grid grid-cols-3 gap-2 text-center mb-4">
                  <div><div className="text-white/40 text-[11px]">Standard MOQ</div><div className="font-semibold text-sm">500 units</div></div>
                  <div><div className="text-white/40 text-[11px]">Min Buy Qty</div><div className="font-semibold text-sm">100 units</div></div>
                  <div><div className="text-white/40 text-[11px]">Active Exceptions</div><div className="font-semibold text-sm">2 active</div></div>
                </div>
                <div className="space-y-2 text-xs">
                  <div className="flex justify-between"><span>Pacific Supplies — MOQ reduced to 100</span><span className="text-white/40">Until 09/30</span></div>
                  <div className="flex justify-between"><span>NorthStar — mix pallet allowed</span><span className="text-white/40">Until 11/15</span></div>
                </div>
              </div>
            </Reveal>
          </div>
          ...
          <div className="space-y-6">
            <Reveal delay={0.05}>
              <div className="card p-5">
                <div className="flex justify-between items-center mb-1">
                  ...
                  <button className="text-xs text-white/40">Filter</button>
                </div>
                <p className="text-white/40 text-xs mb-4">Recent purchase orders and receipts for this SKU</p>
                <div className="space-y-3">
                  {part.history.map((h) => (
                    <div key={h.po} className="flex justify-between items-center text-xs">
                      ...
                      <span className="text-white/40">Qty {h.qty}</span>
            ...
            <Reveal delay={0.1}>
              <div className="card p-5">
                <div className="flex justify-between items-center mb-1">
                  ...
                  <button className="text-xs text-white/40">Sync</button>
                </div>
                <p className="text-white/40 text-xs mb-4">Orders in-flight for this SKU</p>
                <div className="space-y-4">
                  {part.activeOrders.map((o) => (
                    <div key={o.po} className="flex flex-wrap items-center justify-between gap-2">
                      <div>
                        ...
                        <div className="text-xs text-white/40">ETA: {o.eta} · Supplier: {o.supplier}</div>
                      </div>
                      <div className="flex items-center gap-2">
                        <span className="text-xs text-white/40">Qty: {o.qty.toLocaleString()}</span>
            ...
            <Reveal delay={0.15}>
              <div className="card p-5">
                ...
                <p className="text-white/40 text-xs mb-4">Procurement notes and last actions</p>
          ...
          <div className="space-y-6">
            <Reveal delay={0.1}>
              <div className="card p-5">
                ...
                <p className="text-white/40 text-xs mb-4">Simulate order qty vs. fill probability</p>
                <label className="text-xs text-white/40 block mb-1">Order Quantity</label>
                <input
                  type="number"
                  value={qty}
                ...
                />
                <label className="text-xs text-white/40 block mb-1">Expected Lead Time (days)</label>
                <input
                  type="number"
                  value={leadTime}
                ...
                />
                <div className="text-xs text-white/40 mb-1">Fill Probability</div>
